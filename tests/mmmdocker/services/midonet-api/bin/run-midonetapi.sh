...
# Look if there's attached a volume with custom packages
if ls -la /midonet-api-package/*deb ; then
    echo "Installing midonet-api package from disk..."
    dpkg -i /midonet-api-package/midonet-api*deb
fi

# Look if there's attached a volume with python-midonetclient
if ls -la /python-midonetclient-package/*deb ; then
    echo "Installing python-midonetclient package from disk..."
    dpkg -i /python-midonetclient-package/python-midonetclient*deb
fi
