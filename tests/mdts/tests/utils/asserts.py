...
def async_assert_that(*args):
    ...
    ifname = iface.get_vm_ifname()
    nsname = 'vm'+iface.get_vm_id()
    """ Returns future of assert_that(*args)"""
    f = EXECUTOR.submit(assert_that, *args)

    LOG.debug('Scheduled tcpdump on interface %s on vm: %s' % (ifname, nsname))
    iface._tcpdump_sem.acquire()

    LOG.debug('Assert ready on interface %s on vm %s' % (ifname, nsname))
    return f
